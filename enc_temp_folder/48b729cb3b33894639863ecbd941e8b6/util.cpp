...
glm::vec3 heat_color_calculation(const Vertex& vertex, 
    ...
    case 0:
        ...
        if (scaledCurvature < 0.30) {
            ...
            c = glm::vec3(0.025f, 1.0f, 1.0f - scaledCurvature);
        } else if (scaledCurvature < 0.65) {
            scaledCurvature = (scaledCurvature - 0.5f) * 4.0f;
            c = glm::vec3(scaledCurvature, 0.8f, 0.2f);
        } else if (scaledCurvature < 0.8) {
            scaledCurvature = (scaledCurvature - 0.80f) * 3.0f;
    ...
    case 1:
        scaledCurvature = (vertex.indexed_curv - min) / (max - min);
        if (vertex.indexed_curv < 0) { // (scaledCurvature < 0.5)
            scaledCurvature *= 4.0;
            c = glm::vec3(0.0, scaledCurvature, 1.0f);
        } else if (vertex.indexed_curv <= 1) { // scaledCurvature < 0.6666
            scaledCurvature = (scaledCurvature - 0.5f) * 4.0f;
            c = glm::vec3(0.0f, 1.0f, 1.0f - scaledCurvature);
        } else if (vertex.indexed_curv < 3) { // scaledCurvature < 1
            scaledCurvature = (scaledCurvature - 0.6666f) * 4.0f;
