    ...
    glm::vec3& l_axis) 
{
    ...
    auto length_long_axis = glm::length(l_axis) * glm::length(l_axis); 
    ...
    for (int i = 0; i < current_vertex.ring.size(); i++) {
        ...
        if (abs(glm::dot(ed_vector, l_axis) / length_long_axis)  < 1E-2) {
            continue; 
        }

        // Map both onto the longitudinal axis and find length (nominal strain)
        auto proj_ed_vector = glm::length((glm::dot(ed_vector, l_axis) / length_long_axis) * l_axis); 
        auto proj_es_vector = glm::length((glm::dot(es_vector, l_axis) / length_long_axis) * l_axis); 
        
        // Strain percentage change
        auto l_strain =  100 * ((proj_es_vector - proj_ed_vector) / proj_ed_vector); 
       
        // Subtract two vectors 
        u_vectors.push_back(l_strain); 
    }
    ...
    Strain& strain)
{
    ...
    for (int i = 0; i < vertices_ed.size(); i++) {
        float strain_i = longitudinal_strain_vertex(i, vertices_es, vertices_ed, l_axis);
